...
void SimplexNoise_float(float3 position, float angle, uint octaves, out float noise)
{
    ...
    // Remap required because it's remapping [-1.0, 1.0] -> [0.0, 1.0].
    
    //noise = smoothstep(-1.0, 1.0, noise);
    noise = InverseLerp(-1.0, 1.0, noise);
}
...
void SimplexNoise_Caustics_float(float3 position, float angle, uint octaves, float gradientStrength, out float noise)
{
    ...
    SimplexNoise_float(position, angle, octaves, noise, gradient);
    SimplexNoise_float(position - (gradient * gradientStrength), angle, octaves, noise);
}
...
{    
    SimplexNoise_float(position, angle, octaves, noise, gradient);
    SimplexNoise_float(position - (gradient * gradientStrength), angle, octaves, noise, gradient);
}

// Twist around Y axis (2D).

float2 TwistY2(float2 position, float2 center, float angle)
{
...
void SimplexNoise_Caustics_PolarCoordinates_float(float3 position, uint octaves, float gradientStrength, float scale, float3 tiling, float4 animation, float4 offset, float time, bool simplePolarCoordinates, out float noise, out float3 gradient)
{
    float3 polarPosition = position;
    float4 totalOffset = (animation * time) + offset;
    
    // First noise sample.
    
    if (simplePolarCoordinates)
    {
        PolarCoordinates_float(polarPosition.xy, polarPosition.xy);
    }
    else
    {        
        PolarCoordinates2_float(polarPosition.xy, polarPosition.xy);
    }
        
    polarPosition *= scale;
    polarPosition *= tiling;        
    
    float3 offsetPosition = polarPosition + totalOffset.xyz;
    float offsetAngle = totalOffset.w;
            
    SimplexNoise_Caustics_float(offsetPosition, offsetAngle, octaves, gradientStrength, noise, gradient);
    
    // Second noise sample. Exactly the same, but rotated.
    
    polarPosition = position; // Reset.
    polarPosition.xy = RotatePoint(polarPosition.xy, 90.0 * DEG2RAD);
    ...
    offsetPosition = polarPosition + totalOffset.xyz;
    offsetAngle = totalOffset.w;    
    
    float noise_rotated;
    float3 gradient_rotated;
        
    SimplexNoise_Caustics_float(offsetPosition, offsetAngle, octaves, gradientStrength, noise_rotated, gradient_rotated);
        
    // Angular/'cone' mask for blending between rotated and unrotated noise.
        
    float polarCoordinateMaskAngle = -90.0;
        
    float polarCoordinateMask = atan2(position.y, position.x);
    polarCoordinateMask = sin(polarCoordinateMask + (polarCoordinateMaskAngle * DEG2RAD));
        
    // Tighten mask.
        
    polarCoordinateMask = saturate(polarCoordinateMask);
    polarCoordinateMask = pow(polarCoordinateMask, 40.0);
    
    if (!simplePolarCoordinates)
    {
        polarCoordinateMask = 1.0 - polarCoordinateMask;
    }
        
    // Blend noise samples using polar coordinates seams/edge mask.
    
    noise = lerp(noise, noise_rotated, polarCoordinateMask);
    gradient = lerp(gradient, gradient_rotated, polarCoordinateMask);
    
    //noise = noise_rotated;
    //gradient = gradient_rotated;
    
    //noise = polarCoordinateMask; // Debug, show mask.
}
